...
TBL: Shapes are open. You just have to put useful information about stuff people put in their portocol.. we do not nhave to be complete. It is quite reasonable for the OIDC people to make a separate spec - ask what they need exactly for authentication and point to that.
JZ: that makes sense to me. What about - are we making a shape for documents for the profile at all? Where does teh storage go? Is it a shape for the entire graph or for individual documents?
TBL: by default it is on a document. Having the lang in profile
...
SC: you are describing teh RDF graph, if it is in teh same document or in anotehr, it is separate them in documents for some reasons. And you can indicate that - when you have access control as a consideration you would want to. But the data model is for  document - you do not have to specify a uri template, or storage. But gfor the data model you do not describe the value only how data is connected
...
SC: I want to know if the preference file - there is no connection between the prefFile document between anythign described. Example (even if not correct): in profile doc we have a primaryTopic linking to webID but there is no connections between pref file doc with some of teh aprts in it - how do you wire teh webID description to a prefFile? you do not have to describe the conenction
TBL: you do. there is link from profile to a prefFile.
SC: i see config file but nothing else.
JZ: :preferenceFile should be in every profile.
SC: we had several meetings how to..
TBL: I am worried we go a rabbit hole. Lets focus on the profile. The link to the prefFile is very well defined.
...
SC: we approached how to approach the conformance and the spec. (scope, cardinality, relations...) Recently in teh Solid Protocol I made some PR that it really needs to be cristal clear - what will be tested? in the tests of who is conforming? I know we are not but are we testing applications in the spec?
...
SC: if teh spec is about the data model - say that up front. Have a conformance sections - what needs ot be implemented. You can have a producer or consumer of profile. But the shape will be eventually conforming. If producer is creating it well and if consumer is consuming it well.
TBL: we will do just a little bit.
JZ: I think it is possible to create a shape that validate something like - this is a valid profile. But teh profile cannot work under certain conditions and that is what we should describing.
...
TBL: if you have a test for a consent - that is important, you gotta define what is right. The system works - teh app gets access to data. You can have a ACL file but no info at all. If you have authorization you can test the data type. A user creating a ACL file - we do not have that.
...
TBL: this is like about a homepage. you do not have a spec on what to put on teh homepage.
...
TT: A bit about what everyone said: initially when you showed the example it made me frustrataed that you didn't say from the start.
VB: He did!
...
JZ: I am in fav of separating use cases but my concern is what we say in the shapes and that we will need some rules in additiona to shapes. Try to get the shape and see what we need after that.
...
VB: I agree.
JZ: what this leaves for teh uses cases it is to cover that profiles not on Solid servers ..
